 ...
 *            ---
 *            ---
 *             => catch (Exception e) => 모든 예외를 캐치
 *            사용자로부터 문자열을 받아서 정수로 변경 후에 배열에 저장
 *                         aaa                              
 *            나누기를 한 후에 출력
 *     = 예외발생시 => 회피
 *       throws
 ...
 *       문자열 => 정수형으로 변경                        윈도우 / 웹 =>
 *   java.sql => 네트워크 통신                            => 주소란
 *    자바응용프로그램 <============> 오라클              main.jsp?page=1
 *         (C)                          (S)               ----------------
 *       Client                        Server              => 오류 " 1"
 *     => 웹에 출력하는 모든 데이터 => 오라클            NullPointerException
 *     => 자바  ====================== 오라클              => 객체 초기화가 안된 상태
 *                 자바전송(X)                             => 참조할 주소가 없는 경우 (선언)
 *         오라클이 인식할 수 있는 언어 (SQL)            ArrayIndexOutOfBoundsException
 *         SELECT * FROM 테이블명                          => 배열범위 초과시 발생
 *      브라우저 ====== 자바 ====== 오라클               ArithmeticException
 *        HTML                        SQL  => JSP          => 0으로 나눌 경우
 *      ClassNotFoundException(리플렉션)                 ClassCaseException
 *        => 클래스명이 틀린 경우                          => class형변환이 틀린 경우
 *        Class.forName("패키지.클래스명")               IllegalArgumentException
 *           ...                                           => 부적절한 인자 전달
 ...
 *  263page
 *  ---------
 *  예외 의미 => 프로그램에서 돌발적으로(의도하지 않은 상황)에서 문제가 발생시 처리를 어떻게 할지
 *                                       ------------------                    ------------------
 *                                                                             프로그램은 에러시에 비정상으로 종료
...
//		}
		...
		try {
			Class.forName("com.sist.exception예외처리_1");
		} catch (Exception e) {}
			try {
				Thread.sleep(1000);
