 ...
 *                컴파일시애 에외처리여부 학인   |          예외처리여부를 미확인 
 *                => 예외처리를 반드시 한다      |          => 자주 오류나는 부분은 예외로 처리하는 것이 좋다
 ...
 *              InterruptedException (쓰레드)                      NumberFormatException    
 *              MalforedURLException (ip, 웹주소)                  ArrayIndexOutOfBoundsException          
 *              SQLException (문법 => 오라클)                      ClassCastException
 *               
 *              2) 예외처리의 종류
 *                  = ***예외복구 (프로그래머가 주로 사용)
 *                    try {
 ...
 *                      ===> 1) 선언
 *                           2) try~catch
 *                      ex) public void method() throws 예외처리 클래스, 예외처리 클래스... {
 *                                                      ----------------------------------- 순서가 없다
 *                                                      method호출시에 예상되는 에러 체크
 *                          }
 *                          
 *                          ==> 메소드 호출
 *                          public void method() throws 예외처리 클래스, 예외처리 클래스... {
 *                                                      ----------------------------------- 순서가 없다
 *                                                       method호출시에 예상되는 에러 체크
 *                            method()
 *                          }
 ...
 *                => 다른 프로그램과 연동
 *                   브라우저, 오라클...
 *                => 프로그램인데 다른 프로그램을 동시 동작
 *                => 자료구조 (메모리 관리)
 *                ---------------> 직접 만들어서 제공 (라이브러리)
 *                ---------------> 사용자 정의 클래스 + 라이브러리 => 조합
 *     9장/10장 => 라이브러리
 *     ----------------------
 *     java.lang : 자바 프로그램의 기본 라이브러리 (가장많이 사용)
 *                 => import를 생략
 *         라이브러리 => 변수는 없다 / 메소드 (기능)
 *         => Object
