...
public class Server implements Runnable {
	...
	/*
	 * Vector => ArrayList(데이터베이스 읽기) ------ ---------- 동기화 비동기화 | 네트워크
	 * 
	 * add , get , size
	 */
	...
	/*
	 * 내부 클래스 : 클래스끼리 공유하는 변수/메소드가 있는 경우 서버 프로그램 / 빅데이터 프로그램 class Server { class
	 * Client { } }
	 */
				...
				try {
					...
					case Function.INFO: {
						...
						for (Client user : waitVc) {
							// 정보 볼 대상을 찾는다
							/*
							 * 서버의 역할 1) 저장 (클라이언트 정보) => waitVc(Vector) 2) 검색 : ID, Name 3) 수정 : ID, PWD...
							 * 4) 클라이언트로 전송 기능 5) 요청에 처리 기
							 */
