...
// 1. 캡슐화
// => 데이터 보호
// => 데이터를 은닉화 (private) / 메소드를 통해서 접근이 가능 (다른 클래스 연결 => pu
 ...
 * 2) 생성자는 없어도 상관없다, 없는 경우 컴파일러가 자동으로 기본생성자를 추가한다
 * 3) 클래스에는 생성자가 반드시 하나 이상 있어야 한다. (X) 
 *    => 생성자는 업어도 상관없다, 없는 경우 컴파일러가 자동으로 기본생성자를 추가한다
 * 4) 생성자가 없는 클래스는 컴파일러가 기본 생성자를 추가한다. 
 *    같은 이름의 메소드를 여러개 만들어서 사용 = 여러개를 만들 수 있다
 * 5) 생성자는 오버로딩을 할 수 있다
 *    *** 오버로딩의 조건
 *    = 한개의 클래스나 상속 관계의 클래스
 *    = 메소드명 동일
