...
public class StudentSystem {
	 ...
	 * ------------------------------------------------- 2) 생성자 이용 3) 초기화 블록 4) 메소드
	 * -------------- 입력값을 받거나 외부에 데이터 읽기 ------------------ 크롤링, 파일 ** class영역에서는
	 * 선언과 동시에 초기화 --------- 제어문, 외부 라이브러리 호출 불가능 {}
	 */
	...
	public void init() {
		...
		for (int i = 0; i < std.length; i++) {
			...
			std[i] = new Student(); // 배열은 모든 저장 공간이 null(메모리 연결)
			// -------------- 메모리 연결하는 부분
	...
	public void display() { // default => com.sist.main4안에서만 접근이 가능
		for (Student s : std) {
			System.out.println(s.getHakbun() + " " + s.name + " " + s.getKor() + " " + s.getEng() + " " + s.getMath()
					+ " " + s.getSex() + " " + s.getTotal() + " " + s.getAvg() + " " + s.getScore() + " "
					+ s.getRank());
		}
	}
