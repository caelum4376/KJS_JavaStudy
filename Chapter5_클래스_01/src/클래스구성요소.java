 ...
 *        --------------------------
 *        멤버변수
 *          = 인스턴스 변수 => new를 사용할때 마다 메모리가 따로 생성
 ...
 *    ===> 예외처리
 *    ------------------------------------------- 자바언어 기초
 *    ===> 라이브러리 : 사용자정의, 자바에서 지원, 외부업체에서 지원 (Jsoup)
 *    
 *    1. 메소드 기초
 *    -----------
 *    메소드 : 한 개의 기능을 수행할 목적
 ...
 *            -----구구단을 출력
 *            
 *            return; ==> 메소드는 종료시점이 retrun에서 종료
 *            => void일때는 생략이 가능 => 생략을 하면 컴파일러가 자동으로 return추가
 *           
 *       }
 *       
 *       **변수(20%) / 메소드(80%)
...
public class 클래스구성요소 {
	...
	// 총점계산 메소드
	static int totalCalc(int kor, int eng, int math, int hi, int phy) {
		return kor+eng+math+hi+phy;
	}
	
	// 평균계산 메소드
	static double avgCalc(int total) {
		return total/5.0;
	}
	
	// 학점계산 메소드
	static char gradeCalc(double avg) {
		char score = 'A';
		switch ((int)(avg/10)) {
		case 10:
		case 9:
	...
	public static void main(String[] args) {
		...
		// 총점 계산 메소드 호출
		int total = totalCalc(kor, eng, math, hi, phy);
		System.out.println("총점 : " + total);
		
		// 평균 계산 메소드 호출
		double avg = avgCalc(total);
		System.out.printf("평균 : %.2f\n", avg);
		
		// 학점 계산 메소드 호출
		char score = gradeCalc(avg);
