 ...
 *                                       반드시 예외처리를 해야 된다
 *                 2) UnCheckException : 컴파일시에 예외처릴를 확인하지 않는다
 *                                       필요시에만 예외처리
 *                 Exception
 *                     |
 *       ----------------------------------------------------------------
 ...
 *            (java.net)
 *         => ClassNotFoundException : 클래스이름으로 메모리 할당 => 클래스명, 패키지명이 틀린 경우
 *            => 스프링에서 주로 사용하는 기술 (리플렉션)
 *                                              -------- private까지 접근이 가능
 *            A a = new A(); Class.forName("A")
 *         => InterruptedException : 쓰레드 (한 개의 프로세스 안에서 여러개의 프로그램을 동시에 사용)
 *            충돌과 관련
 *         => java.lang / java.util => 예외처리와 관련 없다
 *         => 예외처리는 필수 : 정상상태를 유지 => 견고한 프로그램
 *       2) UnCheckedException : 예외처리는 필요한 경우에만 사용
 *                |
 *            RuntimeException : 실행시 (인터프리터) => 사용자 입력, 프로그래머 실수
 *                |
 ...
 *            NullPointerExcepton : 객체나 배열에 주소가 없는 경우
 *            ClassCastException : 클래스 형변환이 잘못한 경우
 * 예외처리의 기본 (267page)
 * try - catch
 * ------------
 * 형식)   try {
